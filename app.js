...
function boot(){
  ...
  INITIAL_PN = m ? decodeURIComponent(m[1].replace(/\+/g,' ')).replace(/["']/g,'').trim() : '';
...
function renderList(){
  ...
  if (curMatches.length > shownLimit){
    html += '<button class="more" onclick="showMore()">Show more (' + (curMatches.length - shownLimit) + ' more)</button>';
  }
  view.innerHTML = html;
  loadImagesFor(slice);
}
...
function buildCard(p, idx){
  ...
  + '  <div class="seg">'
  + '    <button class="take' + (mode==='take'?' on':'') + '" onclick="setModeCard(' + idx + ',\'take\')">➖ Take out</button>'
  + '    <button class="add' + (mode==='add'?' on':'') + '" onclick="setModeCard(' + idx + ',\'add\')">➕ Add / restock</button>'
  + '  </div>'
  + '  <div class="stepper">'
  + '    <button onclick="bumpCard(' + idx + ',-1)">−</button>'
  + '    <input id="qty-' + idx + '" type="number" inputmode="numeric" value="1" min="1" oninput="updLabel(' + idx + ')">'
  + '    <button onclick="bumpCard(' + idx + ',1)">+</button>'
  + '  </div>'
  + '  <input class="notes" id="notes-' + idx + '" type="text" placeholder="Notes (optional) — e.g. tail #, work order">'
  + '  <button class="submit ' + mode + '" id="submit-' + idx + '" onclick="submitCard(' + idx + ')">'
  ...
  + '  <div class="cardfoot">'
  +      '<button class="editlink" onclick="openEdit(' + idx + ')">Edit count / reorder point</button>'
  +      '<button class="dellink" onclick="askDeleteCard(' + idx + ')">Delete part</button></div>'
  + '</li>';
}
...
function renderManage(){
  ...
  view.innerHTML = ''
    ...
    + '  <button class="btn-primary" id="addBtn" onclick="doAddPart()">Add to inventory</button>'
    + '</div>'
    + '<div class="panel">'
...
function openEdit(idx){
  ...
  card.querySelector('.cardfoot').innerHTML = ''
    ...
    + '  <div class="btns">'
    + '    <button class="cxl" onclick="closeEdit(' + idx + ')">Cancel</button>'
    + '    <button class="save" id="e-s-' + idx + '" onclick="saveEdit(' + idx + ')">Save</button>'
    + '  </div>'
    + '</div>';
...
function footHtml(idx){
  return '<button class="editlink" onclick="openEdit(' + idx + ')">Edit count / reorder point</button>'
       + '<button class="dellink" onclick="askDeleteCard(' + idx + ')">Delete part</button>';
}
...
function askDeleteCard(idx){
  ...
  card.querySelector('.cardfoot').innerHTML = '<div class="confirm">'
    ...
    + '</p><div class="btns">'
    + '<button class="no" onclick="cancelDelete(' + idx + ')">Cancel</button>'
    + '<button class="yes" onclick="doDeleteCard(' + idx + ')">Yes, delete</button>'
    + '</div></div>';
}
...
function thumbHtml(p, idx){
  ...
  if (u){
    return '<div class="thumb has" id="thumb-' + idx + '" onclick="openPhoto(' + idx + ')"'
         + ' title="Tap to view full size">'
         + '<img src="' + esc(u) + '" alt="">'
         + '<span class="cam">🔍</span></div>';
  }
  return '<div class="thumb" id="thumb-' + idx + '" onclick="pickPhoto(' + idx + ', true)"'
...
function askDeletePhoto(){
  ...
  document.getElementById('pmConfirmSlot').innerHTML = ''
    ...
    + '  <div class="btns">'
    + '    <button class="no" onclick="cancelDeletePhoto()">Cancel</button>'
    + '    <button class="yes" onclick="doDeletePhoto()">Yes, delete</button>'
    + '  </div>'
    + '</div>';
}
...
function renderReorder(){
  ...
  var html = '<button class="copybtn" onclick="copyOrderList()">Copy order list</button>';
...
function renderLabels(){
  ...
  view.innerHTML = ''
    ...
    + '  <button onclick="addLabelFromInput()">Add</button>'
    + '</div>'
    + '<div class="lblbar noprint">'
    + '  <button onclick="addAllLabels()">Add ALL parts</button>'
    + '  <button class="ghost" onclick="addLowLabels()">Add low-stock only</button>'
    + '  <button class="ghost" onclick="addBlankLabel()">Blank label</button>'
    + '  <button class="danger" onclick="clearLabels()">Clear</button>'
...
function renderYear(){
  ...
  var html = '<button class="copybtn" onclick="exportYearPdf()">Export as PDF</button>';
  ...
  if (d.rows.length > yearLimit){
    html += '<button class="more" onclick="yearMore()">Show more (' + (d.rows.length - yearLimit) + ' more)</button>';
  }
  view.innerHTML = html;
}
